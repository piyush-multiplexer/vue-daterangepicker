<template>
  <div class="calendar-root">
    <div class="input-date" @click="toggleCalendar()"> {{getDateString(dateRange.start)}} -
...
<script>
  ...
  const defaultPresets = function (i18n = defaultI18n) {
    return {
      ...
      thisWeek: function () {
        const startToday = new Date();
        const endToday = new Date();
        startToday.setTime(startToday.getTime() - 3600 * 1000 * 24 * 7);
        return {
      ...
      thisQuarter: function () {
        ...
        startMonth.setTime(startMonth.getTime() - 3600 * 1000 * 24 * 90);
        return {
          label: presetRangeLabel[i18n].thisQuarter,
          active: false,
          dateRange: {
            start: startMonth,
            end: endMonth
      ...
      lastYear: function () {
        const startMonth = new Date();
        const endMonth = new Date();
        startMonth.setTime(startMonth.getTime() - 3600 * 1000 * 24 * 365);
        return {
      ...
      custom: function () {
        return {
          label: presetRangeLabel[i18n].custom,
          active: false
        }
      }
    }
  ...
  export default {
    ...
    created() {
      ...
      // this.dateRange.start = commonConstants.tempStartdate
      // this.dateRange.end = commonConstants.tempEnddate
    ...
    methods: {
      ...
      setDateValue: function () {
        console.log('awjef',this.dateRange)
